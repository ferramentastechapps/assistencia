...
async function generateOrderNumber(tenantId) {
    const lastOrder = await prisma_1.prisma.repairOrder.findFirst({
        where: { tenantId },
...
async function repairsRoutes(app) {
    ...
    app.get('/repairs', opts, async (request, reply) => {
        ...
        const [total, orders] = await Promise.all([
            prisma_1.prisma.repairOrder.count({ where }),
    ...
    app.get('/repairs/stats', opts, async (request, reply) => {
        ...
        const [total, emBancada, prontos, entregues, totalRevenue] = await Promise.all([
            prisma_1.prisma.repairOrder.count({ where: { tenantId } }),
            prisma_1.prisma.repairOrder.count({ where: { tenantId, status: { in: ['EM_ANALISE', 'EM_CONSERTO', 'AGUARDANDO_PECA'] } } }),
            prisma_1.prisma.repairOrder.count({ where: { tenantId, status: 'PRONTO' } }),
            prisma_1.prisma.repairOrder.count({ where: { tenantId, status: 'ENTREGUE' } }),
            prisma_1.prisma.repairOrder.aggregate({
    ...
    app.get('/repairs/track/:query', async (request, reply) => {
        ...
        if (!tenantId)
            return reply.code(400).send({ error: 'tenantId obrigatório' });
        ...
        if (!order)
            return reply.code(404).send({ error: 'Ordem de serviço não encontrada' });
        return reply.send({
    ...
    app.get('/repairs/quotes', opts, async (request, reply) => {
        const tenantId = request.user.tenantId;
        const { brand, service } = request.query;
    ...
    app.post('/repairs', opts, async (request, reply) => {
        ...
        const order = await prisma_1.prisma.repairOrder.create({
            data: {
                tenantId,
                orderNumber,
    ...
    app.patch('/repairs/:id', opts, async (request, reply) => {
        ...
        const tenantId = request.user.tenantId;
        const body = request.body;
        const existing = await prisma_1.prisma.repairOrder.findFirst({ where: { id, tenantId } });
        if (!existing)
            return reply.code(404).send({ error: 'Ordem não encontrada' });
        ...
        if (body.approvedAt)
            data.approvedAt = new Date(body.approvedAt);
        if (body.paidAt)
            data.paidAt = new Date(body.paidAt);
        if (body.deliveredAt)
            data.deliveredAt = new Date(body.deliveredAt);
        const updated = await prisma_1.prisma.repairOrder.update({
            where: { id },
            data
        });
    ...
    app.patch('/repairs/:id/status', opts, async (request, reply) => {
        const { id } = request.params;
        const tenantId = request.user.tenantId;
        const { status, notifyWhatsapp = true, customMessage } = request.body;
        const order = await prisma_1.prisma.repairOrder.findFirst({ where: { id, tenantId } });
        if (!order)
            return reply.code(404).send({ error: 'Ordem não encontrada' });
        const updateData = { status };
        if (status === 'ENTREGUE')
            updateData.deliveredAt = new Date();
        const updated = await prisma_1.prisma.repairOrder.update({ where: { id }, data: updateData });
            ...
            try {
                ...
                if (instance) {
                    ...
                    if (message) {
                        ...
                        await evolution_client_1.evolutionClient.sendText(instance.instanceName, phone, message);
                        whatsappSent = true;
                        if (status === 'PRONTO') {
                            await prisma_1.prisma.repairOrder.update({ where: { id }, data: { notifiedReady: true } });
                        }
                    }
                }
            }
    ...
    app.delete('/repairs/:id', opts, async (request, reply) => {
        ...
        if (!existing)
            return reply.code(404).send({ error: 'Ordem não encontrada' });
        await prisma_1.prisma.repairOrder.delete({ where: { id } });
    ...
    app.post('/repairs/quotes', opts, async (request, reply) => {
        const tenantId = request.user.tenantId;
        const body = request.body;
        const quote = await prisma_1.prisma.repairQuoteCatalog.create({
            data: { tenantId, ...body }
        });
    ...
    app.delete('/repairs/quotes/:id', opts, async (request, reply) => {
        const { id } = request.params;
        const tenantId = request.user.tenantId;
        await prisma_1.prisma.repairQuoteCatalog.deleteMany({ where: { id, tenantId } });
        return reply.code(204).send();
    });
}
