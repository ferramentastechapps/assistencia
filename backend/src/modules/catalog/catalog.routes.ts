...
const TRADE_IN_BASE_VALUES: Record<string, Record<string, number>> = {
  'Apple': {
...
function calculateTradeIn(
  ...
  if (brandData) {
    ...
    const partialKey = Object.keys(brandData).find(k =>
  ...
  if (baseValue === 0) baseValue = 500 // Fallback para modelos desconhecidos
  ...
  if (storage?.includes('256')) value *= 1.08
  if (storage?.includes('512')) value *= 1.18
  if (storage?.includes('1T') || storage?.includes('1TB')) value *= 1.30

  // Bônus por acessórios
  if (hasBox) value *= 1.03
  if (hasCharger) value *= 1.02
...
export async function catalogRoutes(app: FastifyInstance) {
  ...
  app.get('/catalog/products', opts, async (request, reply) => {
    ...
    const {
      brand, category, condition, search,
      available, featured, page = '1', limit = '20'
    ...
    const [total, products] = await Promise.all([
      prisma.phoneProduct.count({ where }),
      prisma.phoneProduct.findMany({
        where,
  ...
  app.get('/catalog/stats', opts, async (request, reply) => {
    ...
    const [totalProducts, totalStock, available, featured, avgPrice] = await Promise.all([
      prisma.phoneProduct.count({ where: { tenantId } }),
      prisma.phoneProduct.aggregate({ where: { tenantId }, _sum: { stock: true } }),
      prisma.phoneProduct.count({ where: { tenantId, isAvailable: true, stock: { gt: 0 } } }),
      prisma.phoneProduct.count({ where: { tenantId, isFeatured: true } }),
      prisma.phoneProduct.aggregate({ where: { tenantId }, _avg: { cashPrice: true } }),
    ])
  ...
  app.get('/catalog/products/:id', opts, async (request, reply) => {
    ...
    const product = await prisma.phoneProduct.findFirst({ where: { id, tenantId } })
  ...
  app.post('/catalog/products', opts, async (request, reply) => {
    ...
    const product = await prisma.phoneProduct.create({
      data: {
        tenantId,
        title: body.title,
        brand: body.brand,
        model: body.model,
        category: (body.category as any) || 'SMARTPHONE',
        condition: (body.condition as any) || 'SEMINOVO_A_PLUS',
  ...
  app.patch('/catalog/products/:id', opts, async (request, reply) => {
    ...
    const existing = await prisma.phoneProduct.findFirst({ where: { id, tenantId } })
    ...
    const updated = await prisma.phoneProduct.update({ where: { id }, data })
  ...
  app.delete('/catalog/products/:id', opts, async (request, reply) => {
    ...
    await prisma.phoneProduct.delete({ where: { id } })
  ...
  app.post('/catalog/trade-in', opts, async (request, reply) => {
    ...
    const { min, max } = calculateTradeIn(
      body.brand,
      body.model,
      body.storage ?? null,
      body.batteryHealth ?? null,
      body.condition,
      body.hasBox ?? false,
      body.hasCharger ?? false
    )

    // Salvar simulação
    const simulation = await prisma.tradeInSimulation.create({
      data: {
        tenantId,
        brand: body.brand,
        model: body.model,
  ...
  app.get('/catalog/trade-in', opts, async (request, reply) => {
    ...
    const simulations = await prisma.tradeInSimulation.findMany({
      where: { tenantId },
