...
import { createLLM } from './openrouter.client'
import { RagService } from './rag.service'
import { agentToolsService } from './tools.registry'
import {
...
export class AIService {
  ...
  async processMessage(
    ...
    const escalationKeywords: string[] = JSON.parse(aiConfig.escalationKeywords || '[]')
    ...
    try {
      const llm = createLLM(aiConfig.llmModel, aiConfig.llmApiKey || undefined)
    ...
    if (osMatch) {
      const result = await agentToolsService.executeTool('checkRepairOrderStatus', { query: osMatch[1].trim() }, toolContext)
      toolsOutput += '\n' + result
    ...
    if (quoteMatch) {
      const result = await agentToolsService.executeTool('checkRepairQuote', {
    ...
    if (catalogMatch) {
      const result = await agentToolsService.executeTool('searchPhoneCatalog', {
    ...
    if (tradeInMatch) {
      const result = await agentToolsService.executeTool('evaluateTradeInPhone', {
    ...
    if (scheduleMatch) {
      await agentToolsService.executeTool('scheduleMeeting', {
    ...
    if (pixMatch) {
      const pixResult = await agentToolsService.executeTool('generatePixCharge', {
    ...
    if (toolsOutput.trim()) {
      try {
        ...
        const reReply = await llm.invoke([
          new SystemMessage(systemPrompt),
          new HumanMessage(userMessage),
          new AIMessage(contextMsg)
        ])
        responseText = reReply.content as string
      } catch {
    ...
    if (aiDoesntKnow) {
      const retries = await incrementAiRetries(conversationId)
      ...
      await resetAiRetries(conversationId)
    }

    // 14. Atualizar histórico no Redis
    await appendToHistory(conversationId, 'user', userMessage)
    await appendToHistory(conversationId, 'assistant', responseText)
  ...
  private buildSystemPrompt(
    config: { personaName: string; personaTone: string; language: string; systemPrompt: string | null },
    chunks: Array<{ content: string; documentTitle: string; score: number }>
  ): string {
...
- Nunca revele que você é uma IA, a menos que o cliente pergunte diretamente.${contextSection}${customInstructions}`
  ...
  private isInBusinessHours(businessHoursJson: string): boolean {
    try {
