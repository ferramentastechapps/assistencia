...
export class AgentToolsService {
  ...
  async executeTool(
    toolName: string,
    args: any,
    context: { conversationId: string; tenantId: string; instanceName: string; remoteJid: string }
  ): Promise<string> {
    try {
      switch (toolName) {
  ...
  private async handleSchedule(
    args: { date: string; time: string; notes?: string },
  ...
  private async handleRepairQuote(
    args: { brand: string; model: string; serviceType: string },
    ...
    const modelMatch = quotes.find(q =>
      q.modelPattern === '*' ||
      args.model.toLowerCase().includes(q.modelPattern.toLowerCase()) ||
      q.modelPattern.toLowerCase().includes(args.model.toLowerCase())
    ) || quotes[0]
  ...
  private async handleRepairOrderStatus(
    args: { query: string },
    context: { tenantId: string }
  ): Promise<string> {
  ...
  private async handlePhoneCatalogSearch(
    args: { brand?: string; maxPrice?: number; search?: string },
    context: { tenantId: string }
  ): Promise<string> {
    const where: any = { tenantId: context.tenantId, isAvailable: true, stock: { gt: 0 } }
    if (args.brand) where.brand = { contains: args.brand, mode: 'insensitive' }
    if (args.maxPrice) where.cashPrice = { lte: args.maxPrice }
    if (args.search) {
      where.OR = [
    ...
    const products = await prisma.phoneProduct.findMany({
      where,
  ...
  private async handleTradeInEvaluation(
    args: { brand: string; model: string; storage?: string; batteryHealth?: number; condition: string },
    ...
    let base = Object.entries(brandData).find(([k]) =>
      args.model.toLowerCase().includes(k.toLowerCase())
    )?.[1] ?? 500
    ...
    if (args.storage?.includes('256')) value *= 1.08
    if (args.storage?.includes('512')) value *= 1.18

    const min = Math.round(value * 0.90 / 50) * 50
    const max = Math.round(value * 1.10 / 50) * 50

    await prisma.tradeInSimulation.create({
      data: {
        tenantId: context.tenantId,
        brand: args.brand,
        model: args.model,
  ...
  private async handleSendCatalog(
    args: { documentName?: string },
    context: { tenantId: string; instanceName: string; remoteJid: string }
  ): Promise<string> {
  ...
  private async handleGeneratePix(
